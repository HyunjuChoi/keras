...
mcp = ModelCheckpoint(
    ...
    save_best_only=True,                                              # save_best_only: 가중치 가장 좋은 지점 저장!
                                                                      # if you don’t use save_best_only, the default behavior is 
                                                                      # to save the model at the end of every epoch.
...
model.fit(x_train, y_train, epochs=2000, batch_size=10, 
          ...
          verbose=1, callbacks=[es, mcp])           #val_loss를 기준으로 최소값이 n번 이상 갱신 안되면 훈련 중지, chk point 저장                 
