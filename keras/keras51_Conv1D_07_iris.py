...
x_train, x_test, y_train, y_test = train_test_split(
    # shuffle = False로 설정 시 셔플 안돼서 맨 앞부터 차례로 비율 설정되어 같은 값만 뽑힘.
...
#2. modeling
model = Sequential()
model.add(Conv1D(40,2, activation='relu', input_shape=(2, 2)))
model.add(Conv1D(30, 2, padding='same', activation='relu'))
model.add(Conv1D(30, 2, padding='same', activation='relu'))
model.add(Conv1D(20, 2, padding='same', activation='relu'))
model.add(Conv1D(20, 2, padding='same', activation='relu'))
model.add(Flatten())
model.add(Dense(30, activation='sigmoid'))
model.add(Dense(20, activation='relu'))
model.add(Dense(10, activation='linear'))
# 다중 분류!!!   =>y 데이터 종류 개수(class) 0, 1, 2 세개이므로 output layer도 3이다
...
es = EarlyStopping(
    ...
    # max로 설정 시 갱신 안됨. (accuracy 사용 시에는 정확도 높을수록 좋기 때문에 max로 설정)
    mode='min',
    # earlystopping n번 (최저점 나올 때까지 n번 돌림. 그 안에 안 나오면 종료)
    patience=10,
    # 이걸 설정해줘야 종료 시점이 아닌 early stopping 지점의 최적 weight 값 사용 가능
...
#     filepath= filepath + 'k51_iris_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기
# )

model.fit(x_train, y_train, epochs=100, batch_size=1,
          validation_split=0.2, verbose=1, callbacks=[es])

# model.save(path + 'keras51_conv1d_save_model_iris.h5')
...
# argmax: y_predict의 가장 큰 값 => 자리값 뽑아줌
...
# y_test: 정수형, y_predict는 실수형이라 error 남
...
1/28 << conv1d >>
Epoch 00031: early stopping
1/1 [==============================] - 0s 228ms/step - loss: 0.4390 - accuracy: 0.9000
loss:  0.4389624297618866
accuracy: 0.8999999761581421
acc:  0.9
'''
