...
# array 데이터에 0값이 없으면 알아서 0추가해서 컬럼 수가 바뀜 ㅠ
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, shuffle=True, random_state=115, test_size=0.2, stratify=y)
...
model.add(Conv1D(20, 2, activation='relu', padding='same'))
model.add(Conv1D(30, 2, activation='relu', padding='same'))
model.add(Conv1D(20, 2, activation='relu', padding='same'))
model.add(Conv1D(20, 2, activation='relu', padding='same'))
model.add(Flatten())
...
#     filepath= filepath + 'k51_fetch_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기
# )
