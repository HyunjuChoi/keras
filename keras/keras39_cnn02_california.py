...
# #2. modeling
model = Sequential()
model.add(Conv2D(128, (2,2), input_shape=(2, 2, 2), activation='relu'))
model.add(Flatten())
...
mcp = ModelCheckpoint(
    ...
    filepath= filepath + 'k39_cali_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기            
)

model.fit(x_train, y_train, epochs=1000, batch_size=500, validation_split=0.2, verbose=1,
                callbacks=[es, mcp])

model.save(path + 'keras39_cnn_save_model_california.h5')   
