...
#2.modeling
model = Sequential()
model.add(Conv2D(30, (2,2), activation='linear', input_shape=(4, 4, 4), padding='same'))
...
mcp = ModelCheckpoint(
    ...
    filepath= filepath + 'k39_digits_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기            
)

model.fit(x_train, y_train, epochs=1000, batch_size=5,
          validation_split=0.2, verbose=1, callbacks=[es, mcp])

model.save(path + 'keras39_cnn_save_model_digits.h5')   
