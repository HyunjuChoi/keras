...
#2. modeling
model = Sequential()
model.add(Conv2D(40, (2,2), activation='relu', input_shape=(2, 2, 1), padding='same'))
model.add(Conv2D(30, (2,2), activation='sigmoid', padding='same'))
model.add(Flatten())
...
mcp = ModelCheckpoint(
    ...
    filepath= filepath + 'k39_iris_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기            
)

model.fit(x_train, y_train, epochs=100, batch_size=1,
          validation_split=0.2, verbose=1, callbacks=[es, mcp])

model.save(path + 'keras39_cnn_save_model_iris.h5')    
