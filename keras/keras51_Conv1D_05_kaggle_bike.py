...
#2. modeling
model = Sequential()
model.add(Conv1D(128, 2, input_shape=(4,2), activation='relu'))
...
model.add(Dense(1, activation='linear'))            #제일 마지막값은 sigmoid 안돼~!! 마지막 값이 다 0~1로 바뀌니까!


print(x_train.shape, x_test.shape)                  # (7620, 2, 2, 2) (3266, 2, 2, 2)
...
#     filepath= filepath + 'k48_LSTM_bike_' + 'd_'+ date + '_'+ 'e_v_'+ filename                      #파일명 날짜, 시간 넣어서 저장하기            
# )


model.fit(x_train, y_train, epochs=1000, batch_size=10, 
          validation_split=0.2,
          verbose=1, callbacks=[es])           #val_loss를 기준으로 최소값이 n번 이상 갱신 안되면 훈련 중지      


# model.save(path2 + 'keras48_LSTM_save_model_drop_bike.h5')      
...
submission.to_csv(path+'submission_0127_early_minmax_conv1d_bike.csv')
